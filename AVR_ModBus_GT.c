...
uint8_t RcCount, TrCount;  //счетчик принятых/переданных данных
bool StartRec = false;// false/true начало/прием посылки
bool bModBus = false;  //флаг обработки посылки
uint8_t cNumRcByte0; //кол-во принятых байт
uint8_t cNumTrByte0;  //кол-во передаваемых байт
uint8_t cmRcBuf0[MAX_LENGHT_REC_BUF] ; //буфер принимаемых данных
uint8_t cmTrBuf0[MAX_LENGHT_TR_BUF] ; //буфер передаваемых данных
...
#define period_us 100 //количество микросекунд на 1 период глобального таймера
#define count_us (256-period_us) 

//контроль паузы 3,5символа для UART
//t3_5= 3.5*(1+8+1+1)/baud
// если baud >= 19200, то t3_5 = 1.75ms
//для baud = 9600   t3_5 = 4.0ms
uint8_t delay3_5 = 40; // ticks GT, 4ms/0.1ms
uint32_t Next_Time;



//инициализация таймера и UART
...
//запуск таймера глобального времени
GLOBAL_TIME_START();
G_TIME = 0;
